...
def main():
    ...
    ## you could think it as one user's chatgpt chat
        ...
        ## Delete a chat
            ...
            confirm = input(f"Confirm to delete chat \'{chat_name}\' from all chats: {all_chats}, in the \'{CURRENT_ENV}\' environment? (y/n): ")
            if confirm.lower() != 'y':
        ...
        ## Start a new chat
            ...
            if ('--load' in sys.argv): 
                ...
                if (len(sys.argv) == 2): # argument: RTSAI chat
                    ...
                    if (len(sys.argv) > 3) and sys.argv[3][0] != '-': 
                        print_error(f"only can load one chat! (here at lest two: {sys.argv[2:4]})"); return
                    elif (not create_chat(chat_name)): # chat creation failed
                        return
            ...
            if ('--graph' in sys.argv): 
                ...
                available_graphs_chat = [chat[len(f"{ENV_PATH}/envs/{CURRENT_ENV}/graphs/{GRAPH_NAME_KEY}"):] for chat in available_graphs_chat]
                invalid_graphs = []
