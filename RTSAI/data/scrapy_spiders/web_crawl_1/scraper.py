...
class web_crawl_1_Spider(scrapy.Spider):
    ...
    def parse(self, response):
        if response.status == 200: 
            with open(os.path.join("/Users/abc/Desktop/RTSAI/RTSAI/data/scrapy_spiders/web_crawl_1", f"response_{web_crawl_ID()}.html"), 'w', encoding='utf-8') as response_file:
                response_file.write(response.text)
        else: 
            with open(os.path.join("/Users/abc/Desktop/RTSAI/RTSAI/data/scrapy_spiders/web_crawl_1", "html_status_code.txt"), 'r+') as html_status_file: 
