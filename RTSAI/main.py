...
class RTSAI_Boundary(tkinter.Frame): 
    ...
    def boundary_drag_place(self, event): 
        event_x, event_y = event_get_relative_coordinate(event, self.master)
        if (self.direction == 'x'): self.x = event_x + self.event_diff_x; 
        elif (self.direction == 'y'): self.y = event_y + self.event_diff_y; 
...
def RTSAI_chat_window(): 
    ...
    left_panel = RTSAI_Panel(RTSAI_window, name = "Left Panel", background="#aaaaaa", width=window_left_width, height=window_height, x = 0, y = 0)
    right_panel = RTSAI_Panel(RTSAI_window, name = "Right Panel", background="#1E1E1E", width=window_width-window_left_width, height=window_height, x = window_left_width, y = 0)
    left_right_boundary = RTSAI_Boundary(RTSAI_window, left_panel, right_panel, name = "Left Right Boundary", background = "#ffffff")
