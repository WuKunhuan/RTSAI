    ...
    if (pos in pos_tags_to_Patterns.keys()): 
        for grammar in pos_tags_to_Patterns[pos]: 
            ...
            if ((word, grammar) not in forbidden_word_grammar and f"\'{word}\'" not in Grammar_Patterns[grammar]): 
                ...
                if (debug == 0): 
                    print (f"grammar grammar {grammar} appended word: {word}")
...
while (modified): 
    ...
    keys = copy.deepcopy(list(Grammar_Patterns.items()))
...
while (parse_attempts < parse_attempts_total): 
    ...
    result_trees = []
    for tree in PSR_parse(Grammar_Patterns, tokens):
        result_trees.append(str(tree))

    # print  (f"Patterns_grammar: {Patterns_grammar}")
    ...
    result_trees_raw = [translated_grammar_tree_raw(tree, POS_key_str) for tree in result_trees]
    result_trees_raw_simplified = [re.sub(r'\s{2,}', ' ', tree_remove_star(tree)) for tree in result_trees_raw]
