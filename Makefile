...
RTSAI_MAN_DIR := /usr/local/share/man/man1
...
RTSAI: $(PY_FILE)
	...
	@sudo rm -f $(RTSAI_EXEC_DIR)
	@sudo cp $(DIST_DIR)/RTSAI_Main $(DIST_DIR)/RTSAI
	@sudo mv $(DIST_DIR)/RTSAI $(RTSAI_EXEC_DIR)
...
clean:
	...
	@sudo rm -f $(RTSAI_MAN_DIR)/RTSAI.1
	@echo "RTSAI manuals removed from $(RTSAI_MAN_DIR)!"
